...
namespace QueuedInputSystem
{
	public class QueuedKeyboardInput : MonoBehaviour
	{
		...
		/// </summary>
		public void RegisterOnPress( KeyCode key, float priorityId, Action<InputQueue> method, bool isEnabled = true, bool isOneShot = false )
		{
		...
		/// </summary>
		public void RegisterOnRelease( KeyCode key, float priorityId, Action<InputQueue> method, bool isEnabled = true, bool isOneShot = false )
		{
			InputQueue inputQueue;
		...
		/// </summary>
		public void ClearOnPress( KeyCode key, Action<InputQueue> method )
		{
			InputQueue inputQueue;
			if( this.press.TryGetValue( key, out inputQueue ) )
			{
		...
		/// Unregisters a function from running.
		/// </summary>
		public void ClearOnHold( KeyCode key, Action<InputQueue> method )
		{
			InputQueue inputQueue;
			if( this.hold.TryGetValue( key, out inputQueue ) )
			{
		...
		/// Unregisters a function from running.
		/// </summary>
		public void ClearOnRelease( KeyCode key, Action<InputQueue> method )
		{
			InputQueue inputQueue;
			if( this.release.TryGetValue( key, out inputQueue ) )
			{
